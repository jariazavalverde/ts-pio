// Copyright 2020 José Antonio Riaza Valverde.
// Released under the terms of the BSD 3-Clause License.
// https://github.com/jariazavalverde/ts-pio
...
IO.prototype.left = function <A, B>(this: IO<A>, action: IO<B>): IO<A> {
  return new IO(() =>
    this.runIO().then((x: A) => action.then(pure(x)).runIO())
  );
};
...
export const filter = function <A>(
  predicate: (x: A) => IO<boolean>,
): (xs: Array<A>) => IO<Array<A>> {
  return (xs) => {
    let action: IO<Array<A>> = pure([]);
    for (let i = xs.length - 1; i >= 0; i--) {
      action = lift2(
        (flag) => (acc) => flag ? [xs[i]].concat(acc) : acc,
        predicate(xs[i]),
        action,
      );
    }
...
export const replicate = function <A>(
  action: IO<A>,
): (n: number) => IO<Array<A>> {
  return (n) => {
    let actions = [];
...
export const lift2 = function <A, B, C>(
  fn: (x: A) => (y: B) => C,
  a: IO<A>,
  b: IO<B>,
): IO<C> {
...
export const lift3 = function <A, B, C, D>(
  fn: (x: A) => (y: B) => (z: C) => D,
  a: IO<A>,
  b: IO<B>,
  c: IO<C>,
...
export const putStr = function (text: string): IO<void> {
  return new IO(() => Deno.stdout.write(new TextEncoder().encode(text))).void();
};
...
export const getChar = new IO<string>(() => {
  ...
  return Deno.stdin.read(buffer).then(
    (n: number | null) =>
      n ? new TextDecoder().decode(buffer.subarray(0, 1)) : "",
  ).finally(
    () => Deno.setRaw(0, false),
  );
});
...
export const getLine = new IO<string>(() => {
  ...
  return Deno.stdin.read(buffer).then(
    (n: number | null) =>
      n ? new TextDecoder().decode(buffer.subarray(0, n)) : "",
  );
});
...
export const writeFile = function (
  path: string,
): (content: string) => IO<void> {
  return (content: string) =>
    new IO(() =>
      Deno.writeFile(path, new TextEncoder().encode(content), { append: false })
    );
};

// Append the string to the file.
export const appendFile = function (
  path: string,
): (content: string) => IO<void> {
  return (content: string) =>
    new IO(() =>
      Deno.writeFile(path, new TextEncoder().encode(content), { append: true })
    );
};
